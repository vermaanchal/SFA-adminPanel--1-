...
import AddRemoveFrameHook from './add-removeframeHook';

const UpdateFrames = () => {
  const {duration,handleAddFrame, handleUserIdChange,handleDurationChange, handleButtonClick,
   frameId,userId, filter,show,frameOption,handleSelectChange ,handleRemoveFrame} = AddRemoveFrameHook();
  const column = [
    {
      name: "User Id",
      // selector: id,
        ...
        return (
          <select value={frameId} onChange={e => handleSelectChange(e, userId)} className='frameSelect'>
           ...
           {frameOption.map((option) => {
            return(
          <option key={option.frameId} value={option.frameId}>
            {option.frameName}
          </option>
